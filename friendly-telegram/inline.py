...
photo = io.BytesIO(
    requests.get(
        "https://github.com/ichiCat/friendly-telegram/blob/main/friendly-telegram/bot_avatar.png" # noqa: E501, W505
    ).content
)
...
class InlineManager:
    ...
    async def _create_bot(self) -> None:
        ...
        async with self._client.conversation("@BotFather", exclusive=False) as conv:
            ...
            # Set its name to user's name + ichiyeahTG Userbot
            m = await conv.send_message(f"❤️‍Neep Userbot of {self._name}")
            r = await conv.get_response()
        ...
        async with self._client.conversation("@BotFather", exclusive=False) as conv:
            ...
            for row in r.reply_markup.rows:
                for button in row.buttons:
                    if re.search(r"@Neeptg_[0-9a-zA-Z]{6}_bot", button.text, re.I):
